...
def evolve_parameters(
    ...
    *,
    ...
    Args:
        buy_strategy: Strategy key (e.g. "equal_slice", "core_dip_dca").
        sell_strategy: Sell strategy key (e.g. "grid_rebound", "repair_step").
    ...
    buy_fields = _relevant_buy_parameter_fields(buy_strategy)
    sell_fields = _relevant_sell_parameter_fields(sell_strategy, base_inputs, buy_strategy)

    population = _initialize_population(
        buy_strategy, sell_strategy, buy_fields, sell_fields,
        base_inputs, config.population_size,
    )
    ...
    for gen in range(config.generations):
        ...
        while len(next_population) < config.population_size:
            ...
            if random.random() < config.crossover_rate:
                child = _crossover(parent1, parent2, buy_fields, sell_fields, base_inputs)
            else:
                child = parent1

            child = _mutate(child, buy_fields, sell_fields, base_inputs, config.mutation_rate)
            next_population.append(child)

        population = next_population[:config.population_size]
        fitnesses = _evaluate_population(population, fitness_fn, cancel_checker)

    # Build ranked final results
...
def _initialize_population(
    ...
    # Seed 1: Default parameters
    default_ind = Individual(
        buy_strategy=buy_strategy,
        sell_strategy=sell_strategy,
        buy_params=_default_buy_params(buy_strategy, base_inputs),
        sell_params=_default_sell_params(sell_strategy, base_inputs, buy_strategy),
    )
    if default_ind.key not in seen_keys:
        seen_keys.add(default_ind.key)
        population.append(default_ind)

    # Seed 2-4: Best known parameter sets from robust config
    for seed_params in _generate_seeded_params(buy_strategy, sell_strategy, base_inputs, buy_fields, sell_fields):
        ind = Individual(
            buy_strategy=buy_strategy,
            sell_strategy=sell_strategy,
    ...
    while len(population) < population_size and attempts < max_attempts:
        attempts += 1
        ind = _random_individual(buy_strategy, sell_strategy, buy_fields, sell_fields, base_inputs)
        if ind.key not in seen_keys:
            seen_keys.add(ind.key)
            population.append(ind)

    # If we still don't have enough (all random individuals are duplicates),
    # pad with suffix-augmented individuals
    suffix = 0
...
def _crossover(
    ...
    for field in buy_fields:
        buy_params[field] = (
            parent1.buy_params.get(field)
            if random.random() < 0.5
            else parent2.buy_params.get(field)
        )

    sell_params: dict[str, object] = {}
    for field in sell_fields:
        sell_params[field] = (
            parent1.sell_params.get(field)
            if random.random() < 0.5
            else parent2.sell_params.get(field)
        )
