...
class StrategyLabFrontendTest(unittest.TestCase):
    ...
    def test_scorecard_payload_uses_selected_strategy_filters(self):
        ...
        self.assertIn("universe-list", html)
        self.assertIn("universe-row", html)
        self.assertNotIn("universe-table", html)
        self.assertNotIn("<th>启用</th>", html)
        self.assertIn("universe-format-details", html)
        self.assertIn(".universe-warning:empty { display: none; }", html)
    ...
    def test_parameter_lab_page_exposes_full_matrix_worker_flow(self):
        ...
        self.assertIn("renderLeapsStockEstimateCell(estimatedLeapsOutcomeFromSignal(signal))", html)
        self.assertIn("renderLeapsOptionOutcome(outcome, signal)", html)
        ...
        self.assertIn('<option value="all">全部 LEAPS</option>', html)
        self.assertIn('<option value="high" selected>仅高等级</option>', html)
        self.assertIn('<option value="option_estimate_roi_mean">预估期权ROI</option>', html)
        self.assertIn("formatParameterRankScore(group.best_score, rankMethod)", html)
    ...
    def test_parameter_lab_cell_replays_detail_inside_parameter_lab(self):
        ...
        self.assertIn("function buildCellPriceTraces(cell, chartDates)", html)
        self.assertIn("function marketSeriesForSymbol(symbol)", html)
        ...
        self.assertIn("name: `${entry.symbol} 股价`", html)
        self.assertIn("yaxis: 'y2'", html)
        self.assertIn("const tradePrice = Number(trade.price);", html)
        self.assertIn("if (usePriceAxisForTrades && Number.isFinite(tradePrice)) return tradePrice;", html)
        self.assertIn("tradeTrace('buy', '买入', '#ff7e67', 'circle')", html)
        self.assertIn("tradeTrace('sell', '卖出', '#054d76', 'diamond')", html)
        self.assertIn("股票价格: ${number(trade.price)}", html)
        self.assertIn("交易后现金: ${Number.isFinite(cashAfter) ? money(cashAfter) : '--'}", html)
        self.assertIn("交易后现金占比: ${Number.isFinite(cashPctAfter) ? pct(cashPctAfter) : '--'}", html)
        self.assertIn("hovertemplate: '%{text}<extra></extra>'", html)
        self.assertIn("title: {", html)
        self.assertIn("text: '买卖点 / 股价 / 现金余额（交易后）'", html)
        self.assertIn("paper_bgcolor: '#fafafa'", html)
        self.assertIn("gridcolor: '#d7ecf8'", html)
        self.assertIn("yaxis: { title: '组合 / 现金'", html)
        self.assertIn("yaxis2: { title: '股价', overlaying: 'y', side: 'right'", html)
