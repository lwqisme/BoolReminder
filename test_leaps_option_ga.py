...
class StopLossTest(unittest.TestCase):
    """Stop-loss: triggered when option ROI falls below a negative threshold."""

    def _make_prices(self, values: list[float], start_date: date) -> list[tuple[date, float]]:
        return [(start_date + timedelta(days=i), v) for i, v in enumerate(values)]

    def test_stop_loss_triggers_when_roi_below_threshold(self):
        """Stock drops sharply → option ROI ≤ -70% → stop-loss sells configured %."""
        # Entry at 100, price crashes to 50 → option should be deep underwater
        values = [100.0] * 122 + [100.0] + [90.0, 80.0, 70.0, 60.0, 50.0] + [50.0] * 200
        start = date(2025, 1, 1)
        prices = self._make_prices(values, start)
        entry_date = start + timedelta(days=122)
        entry = LeapsEntrySignal(
        ...
        # Normal stages: hold 10, profit > 30%, sell all
        stages = [(10, 30.0, 100.0)]
        trade = compute_sell_ladder(
            entry, prices, stages, expiration_days=190,
            strike_price=110.0,
            stage_loss_threshold_pct=70.0,
            stage_loss_sell_pct=60.0,
        )
        # Should have at least one sell event (the stop-loss)
        self.assertGreaterEqual(len(trade.sell_events), 1,
            "Stop-loss should trigger when ROI drops below -70%")
        # First sell event should be the stop-loss (sells stage_loss_sell_pct%)
        first_sell = trade.sell_events[0]
        self.assertAlmostEqual(first_sell.pct_sold, 60.0)
        self.assertLess(first_sell.roi_pct, -70.0,
            f"Stop-loss ROI should be below -70%, got {first_sell.roi_pct}")


    def test_stop_loss_does_not_trigger_when_roi_above_threshold(self):
        """Stock drops mildly → ROI > -75% → stop-loss does NOT trigger."""
        values = [100.0] * 122 + [100.0] + [98.0] * 200
        start = date(2025, 1, 1)
        prices = self._make_prices(values, start)
        entry_date = start + timedelta(days=122)
        entry = LeapsEntrySignal(
        ...
        trade = compute_sell_ladder(
            entry, prices, stages, expiration_days=190,
            strike_price=110.0,
            stage_loss_threshold_pct=75.0,
            stage_loss_sell_pct=60.0,
        )
        for se in trade.sell_events:
            self.assertGreater(se.roi_pct, -75.0,
                f"Stop-loss should not trigger, got ROI {se.roi_pct}")

    def test_after_stop_loss_remaining_goes_to_expiration_sell(self):
        """After stop-loss sells 60%, remaining 40% goes through normal S3."""
        values = [100.0] * 122 + [100.0] + [50.0] * 200  # crash to 50
        start = date(2025, 1, 1)
        prices = self._make_prices(values, start)
        entry_date = start + timedelta(days=122)
        entry = LeapsEntrySignal(
        ...
        trade = compute_sell_ladder(
            entry, prices, stages, expiration_days=190,
            strike_price=110.0,
            stage_loss_threshold_pct=50.0,
            stage_loss_sell_pct=60.0,
        )
        # First event = stop-loss (60%), remaining sold at expiration cutoff
        self.assertGreaterEqual(len(trade.sell_events), 2)
        total_sold = sum(se.pct_sold for se in trade.sell_events)
        self.assertAlmostEqual(total_sold, 100.0, places=2)

    def test_stop_loss_only_triggers_once(self):
        """Stop-loss triggers once, not repeatedly on subsequent days."""
        values = [100.0] * 122 + [100.0] + [40.0] * 200  # severe crash
        start = date(2025, 1, 1)
        prices = self._make_prices(values, start)
        entry_date = start + timedelta(days=122)
        entry = LeapsEntrySignal(
        ...
        trade = compute_sell_ladder(
            ...
            strike_price=110.0,
            stage_loss_threshold_pct=50.0,
            stage_loss_sell_pct=30.0,
        )
        stop_loss_events = [se for se in trade.sell_events if se.roi_pct <= -50.0]
        # May have 2 events below -50% (stop-loss + expiration sell both negative)
        self.assertGreaterEqual(len(trade.sell_events), 2)
        total_sold = sum(se.pct_sold for se in trade.sell_events)
        self.assertAlmostEqual(total_sold, 100.0, places=2)
